...
fn remove_duplicates(nums: &mut Vec<i32>) -> i32 {
    ...
    while next < nums.len() {
        println!("{:?}, cur:{}, next: {}, pre: {}", nums, cur, next, pre);
        if pre < nums[next] {
            cur += 1;
            pre = nums[next];
            next += 1;
...
fn test_case() {
    ...
    cases.push(TestCase{nums: vec![1], expect: vec![1]});
    cases.push(TestCase{nums: vec![1, 1, 1], expect: vec![1]});
    cases.push(TestCase { nums: vec![1, 1, 1,2], expect: vec![1, 2] });
    cases.push(TestCase { nums: vec![1, 1, 1,2, 2, 3], expect: vec![1, 2, 3] });
    cases.push(TestCase { nums: vec![-1, 0, 1, 1, 1,2], expect: vec![-1, 0, 1, 2] });
    cases.push(TestCase { nums: vec![-3,-3,-2,-1,-1,0,0,0,0,0], expect: vec![-3, -2, -1, 0] });
  
    for item in cases.iter_mut() {
        rm(&mut item.nums, &item.expect)
    }
    println!("PASS ALL")
}


fn rm(nums: &mut Vec<i32>, expect: &Vec<i32>){
